   ...
   public function store(Request $request)
{
    ...
    if ($horseSeuil && $niveau) {
        ...
        // ── Chercher alerte ACTIVE (non acquittée) pour ce capteur ──
        ...
        if ($alerteActive) {
            // ✅ Épisode en cours → juste mettre à jour, PAS de nouvelle alerte
            ...
            // Email si escalade WARNING → CRITIQUE
            if ($niveau === 'CRITIQUE' && $niveauAvant !== 'CRITIQUE') {
                try {
                    Mail::to(env('ALERT_EMAIL', 'admin@usine.local'))
                        ->send(new AlerteMail($alerteActive->fresh(), $capteur->machine));
                } catch (\Exception $e) {
            ...
            // ✅ Pas d'alerte active = nouvel épisode → créer UNE alerte
            ...
            if ($niveau === 'CRITIQUE') {
                try {
                    Mail::to(env('ALERT_EMAIL', 'admin@usine.local'))
                        ->send(new AlerteMail($nouvelleAlerte, $capteur->machine));
                } catch (\Exception $e) {
                    \Log::warning('Email alerte non envoyé : ' . $e->getMessage());
                }
            }
        ...
        // Le prochain hors seuil créera une nouvelle alerte
